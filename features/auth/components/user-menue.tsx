...
export function UserMenu({user,variant='compact',plan,className}:UserMenuProps){
        ...
        <div className={cn("flex w-full items-center justify-between gap-4", className)}>
            <div className="flex items-center gap-3 overflow-hidden">
                <Avatar className="size-9">
                    ...
                    <AvatarFallback className="bg-muted-foreground">{initials}</AvatarFallback>
                </Avatar>
                {variant === 'profile' && (
    ...
    return (
        <DropdownMenu>
            <DropdownMenuTrigger render={<Button variant="ghost" className="h-auto w-auto p-0 hover:bg-transparent" />}>
                {content}
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56">
                ...
                <DropdownMenuSeparator />
                <DropdownMenuItem onClick={handleSignOut} className="text-red-600 focus:text-red-600 focus:bg-red-100 cursor-pointer">
                    Sign Out
                </DropdownMenuItem>
            </DropdownMenuContent>
        </DropdownMenu>
